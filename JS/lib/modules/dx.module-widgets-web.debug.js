...
if (!DevExpress.MOD_WIDGETS_WEB) {
    ...
    (function($, DX, undefined) {
        ...
        DX.registerComponent("dxColorPicker", ui.dxDropDownEditor.inherit({
            ...
            _updateColorHue: function(value) {
                var hue = 360 - Math.round(value * 360 / this._$hueScale.height()),
                    saturation = this._currentColor.hsv.s,
                    value = this._currentColor.hsv.v;
                this._updateColorFromHsv(hue, saturation, value);
                this._$palette.css("backgroundColor", this._currentColor.getPureColor().toHex())
    ...
    (function($, DX, undefined) {
        ...
        var dxMenuBase = ui.CollectionContainerWidget.inherit({
                ...
                _renderImage: function(item, $itemContent) {
                    var $itemImage;
                    if (item.icon)
                        $itemImage = $('<span>').addClass(DX_ICON_CLASS + "-" + item.icon);
                    else if (item.iconSrc)
                        $itemImage = $('<img>').attr('src', item.iconSrc);
                    if ($itemImage)
                        $itemImage.addClass(DX_ICON_CLASS).appendTo($itemContent)
                },
                _renderPopout: function(item, $itemContent) {
                    var $popOutImage;
                    if (this._hasChildren(item)) {
                        $popOutImage = $('<span>').addClass(DX_MENU_ITEM_POPOUT_CLASS);
                        $itemContent.append($popOutImage)
                    }
                },
                ...
                _init: function() {
                    ...
                    this.enableSelection = this.option('allowSelectItem');
        ...
        DX.registerComponent("dxContextMenu", dxMenuBase.inherit({
            _defaultOptions: function() {
                return $.extend(this.callBase(), {
                        items: [],
            ...
            _render: function() {
                this._needToRemoveWidgetClass = !this._element().hasClass('dx-widget');
                this.callBase()
            },
            _renderContentImpl: function() {
                if (this._needToRemoveWidgetClass)
                    this._element().removeClass('dx-widget');
                this._renderContextMenuOverlay();
                this._renderAdditionGraphicsElements();
                this._attachItemHoverEvents();
                this._attachShowContextMenuEvents();
                this._element().addClass(DX_CONTEXT_MENU_CLASS);
            ...
            _renderContextMenuOverlay: function() {
                ...
                $overlayElement = $('<div>').appendTo(this._element());
            ...
            _attachShowContextMenuEvents: function() {
                ...
                this._$element.off(holdEventName).on(holdEventName, $.proxy(function(e) {
                    holdAction({jQueryEvent: e})
                }, this));
                this._$element.off(rightClickEventName).on(rightClickEventName, $.proxy(function(e) {
            ...
            _getOverlayOptions: function() {
                var that = this,
                    ...
                    overlayOptions = {
                        ...
                        position: this.option('position').of ? this.option('position') : {
                            at: 'center',
                            my: 'center',
                            of: this._element()
                        },
            ...
            _closeOnOutsideClickHandler: function(e) {
                if (e.target !== document) {
                    if ($(e.target).hasClass('dx-menu-item-content'))
                        if (this._$element.siblings()[0] === e.target)
                            return false;
                    if ($(e.target.parentElement).hasClass('dx-menu-item-content'))
                        if (this._$element.siblings()[0] === e.target.parentElement)
                            return false;
                    if ($(e.target).siblings().hasClass('dx-menu-item-content'))
                        if (this._$element.siblings()[0] === $(e.target).siblings()[0])
                            return false
                }
                return true
            },
            ...
            _handleItemMouseLeave: function(eventArgs) {
                ...
                if ($subMenuElement.length > 0)
                    that._hideSubMenuGroup($subMenuElement)
            },
            ...
            _hideSubMenuGroup: function($subMenu) {
                var that = this,
                    showTimerId = $subMenu.data('data-show-timer'),
                    hideTimerId = $subMenu.data('data-hide-timer');
                ...
                if (this._isSubMenuVisible($subMenu)) {
                    ...
                    $subMenu.data('data-hide-timer', hideTimerId)
                }
            },
            _showSubMenuGroup: function($subMenu, $itemElement, forceRestartTimer) {
                ...
                if (forceRestartTimer || !this._isSubMenuVisible($subMenu)) {
                    ...
                    $subMenu.data('data-show-timer', showTimerId)
                }
            },
            _getSubMenuPosition: function($rootItem) {
                var isVerticalMenu,
                    ...
                    $rootItemWrapper;
                rtlEnabled = !!this.option('rtlEnabled');
                direction = this.option('direction').toLowerCase();
        ...
        DX.registerComponent("dxMenu", dxMenuBase.inherit({
            _defaultOptions: function() {
                return $.extend(this.callBase(), {
            ...
            _renderSelectedItem: function(item, $itemWrapper) {
                if (this.option('allowSelectItem'))
            ...
            _handleItemMouseEnter: function(eventArg) {
                ...
                showPopupMode = that.option('showPopupMode').toLowerCase();
                if (showPopupMode !== 'onclick' && popupMenu) {
                    clearTimeout(that._hidePopupMenuTimer);
                    clearTimeout(that._showPopupMenuTimer);
                    isHoverStayMode = that.option('showPopupMode') !== 'onhover';
            ...
            _handleItemMouseLeave: function(eventArg) {
                var that = this,
                    ...
                    timeout;
                showPopupMode = that.option('showPopupMode').toLowerCase();
                timeout = that.option('showPopupMode') !== 'onhover' ? DX_MENU_HOVERSTAY_TIMEOUT : DX_MENU_HOVER_TIMEOUT;
                $item = that._getItemElementByEventArgs(eventArg);
                if ($item.data(this._itemDataKey()).disabled)
                    return;
                that._renderItemHotTrack($item, false);
                if (showPopupMode !== 'onclick') {
                    clearTimeout(that._showPopupMenuTimer);
                    clearTimeout(that._hidePopupMenuTimer);
                ...
                if (actionArgs.args.length && actionArgs.args[0]) {
                    ...
                    if (popupMenu)
                        if (popupMenu._overlay.option('visible')) {
                            if (that.option('showPopupMode') === 'onclick')
                                that._hidePopupMenu(popupMenu)
                        }
                        else
                            that._showPopupMenu(popupMenu, $item)
            ...
            _getPopupMenuPosition: function($rootItem) {
                ...
                firstSubMenuDirection = this.option('firstSubMenuDirection').toLowerCase();
                popupMenuPosition = {
                    collision: 'flip',
                    of: $rootItem
                };
                switch (firstSubMenuDirection) {
                    case'leftOrTop':
    ...
    (function($, DX) {
        ...
        var View = ViewController.inherit({
                ...
                render: function($root, options) {
                    ...
                    if (!$element)
                        $element = this._$element = $('<div />').appendTo($root);
                    ...
                    _defaultOptions: function() {
                        ...
                        $.each(dataGrid.modules, function() {
                            if ($.isFunction(this.defaultOptions))
                                $.extend(resultOptions, this.defaultOptions())
                        });
                        return $.extend(this.callBase(), resultOptions)
                    },
                    ...
                    _getTemplate: function(templateName) {
                        ...
                        if (!this._templates[templateName]) {
                            ...
                            if ($template.length === 1) {
                                ...
                                this._templates[templateName] = new templateClass($template)
                            }
                        }
            ...
            return {
                    ...
                    foreachGroups: function(callback, childrenAtFirst) {
                        ...
                        function foreachGroupsCore(groupsInfo, callback, childrenAtFirst, parents) {
                            var i;
                            for (i = 0; i < groupsInfo.length; i++) {
                                if (!childrenAtFirst && groupsInfo[i].data && callback(groupsInfo[i].data, parents) === false)
                                    return false;
                                if (!groupsInfo[i].data || groupsInfo[i].data.isExpanded) {
                                    parents.push(groupsInfo[i].data);
                                    if (foreachGroupsCore(groupsInfo[i].children, callback, childrenAtFirst, parents) === false)
                                        return false;
                                    parents.pop()
                                }
                                if (childrenAtFirst && groupsInfo[i].data && callback(groupsInfo[i].data, parents) === false)
                        ...
                        foreachGroupsCore(that._groupsInfo, callback, childrenAtFirst, [])
                    },
                    ...
                    _collapseExpandAll: function(groupIndex, isExpand) {
                        ...
                        that.foreachGroups(function(groupInfo, parents) {
                            if (groupIndex === undefined || groupIndex === parents.length)
    ...
    (function($, DX) {
        ...
        DX.ui.dataGrid.GroupingHelper = DX.ui.dataGrid.GroupingHelperBase.inherit(function() {
            ...
            var processGroupItems = function(that, items, path, offset, skipFirstItem, take) {
                    ...
                    for (i = 0; i < items.length; i++) {
                        item = items[i];
                        if (utils.isDefined(item.key) && utils.isDefined(item.items)) {
                            path.push(item.key);
            ...
            var updateGroupOffsets = function(that, items, path, offset, additionalGroupInfo) {
                    ...
                    for (i = 0; i < items.length; i++) {
                        item = items[i];
                        if (item.key !== undefined && item.items !== undefined) {
                            path.push(item.key);
            ...
            return {
                    ...
                    changeGroupExpand: function(path) {
                        var that = this,
                            ...
                            groupCountQuery;
                        groupCountQuery = dataSource.store().totalCount({filter: createGroupFilter(path, {
                    ...
                    mergeLoadedData: function(datas, groupDataOptions) {
                        var that = this,
                            ...
                            collapsedPaths,
                            groupsCount = normalizeSortingInfo(groupDataOptions.group || []).length,
                        ...
                        for (i = 0; i < loadsCount; i++) {
                            ...
                            if (datas[i])
                                new DX.data.ArrayStore(datas[i]).load({group: groupDataOptions.group}).done(function(groupedData) {
                    ...
                    refresh: function(storeLoadOptions) {
                        ...
                        that.callBase(storeLoadOptions);
                        foreachCollapsedGroups(that, function(groupInfo) {
                            var groupCountQuery = store.totalCount({filter: createGroupFilter(groupInfo.path, storeLoadOptions)});
                            var groupOffsetQuery = store.totalCount({filter: createOffsetFilter(groupInfo.path, storeLoadOptions)});
                            loads.push(groupCountQuery);
                            loads.push(groupOffsetQuery);
    ...
    (function($, DX) {
        ...
        DX.ui.dataGrid.GroupingHelperWithNativeGrouping = DX.ui.dataGrid.GroupingHelperBase.inherit(function() {
            ...
            var getTotalOffset = function(groupInfo, parents) {
                    var i,
                        totalOffset = groupInfo.offset + 1;
                    for (i = 0; i < parents.length; i++)
                        totalOffset += parents[i].offset + 1;
                    return totalOffset
                };
            var getContinuationGroupCount = function(groupOffset, pageSize, groupSize) {
                    if (pageSize > 1 && groupSize > 1) {
                        var pageOffset = groupOffset - Math.floor(groupOffset / pageSize) * pageSize;
                        return Math.floor((pageOffset + groupSize - 2) / (pageSize - 1))
                    }
                    return 0
                };
            ...
            return {
                    updateTotalItemsCount: function() {
                        ...
                        foreachExpandedGroups(this, function(groupInfo, parents) {
                            var totalOffset = getTotalOffset(groupInfo, parents),
                                count = groupInfo.count,
                                i;
                            if (!isVirtualPaging) {
                                count += getContinuationGroupCount(totalOffset, pageSize, groupInfo.totalCount + count);
                                groupInfo.totalCount += count;
                                for (i = 0; i < parents.length; i++)
                                    parents[i].totalCount += count
                            }
                    ...
                    processLoadOptions: function(storeLoadOptions, isExpandedGroupsLoaded) {
                        var that = this,
                            ...
                            loadOptions = $.extend({}, storeLoadOptions);
                        that.callBase(storeLoadOptions);
                        if (hasExpandedGroup(that, loadOptions.group) && !isExpandedGroupsLoaded) {
                            ...
                            foreachExpandedGroups(that, function(groupInfo, parents) {
                                var skip,
                                    ...
                                    totalOffset = getTotalOffset(groupInfo, parents);
                                if (!isVirtualPaging)
                                    groupInfoCount += getContinuationGroupCount(totalOffset - loadOptions.skip, pageSize, groupInfo.count);
                                if (totalOffset + groupInfoCount <= loadOptions.skip)
                                    loadOptions.skip -= groupInfoCount;
                                else if (totalOffset <= loadOptions.skip + loadOptions.take && groupInfoCount) {
                                    skip = loadOptions.skip - totalOffset;
                                    take = loadOptions.take;
                                    if (!isVirtualPaging && pageSize && skip >= 0) {
                                        take--;
                                        skip -= Math.floor(skip / pageSize)
                                    }
                                    if (totalOffset + groupInfoCount < loadOptions.skip + loadOptions.take)
                                        loadOptions.take -= groupInfoCount - Math.max(loadOptions.skip - totalOffset + 1, 0);
                                    else {
                                        takes.unshift(take + Math.min(skip, 0));
                                        loadOptions.take = Math.max(1, -skip)
                                    }
                                    if (skip >= 0) {
                                        loadOptions.skip = totalOffset - 1;
                                        skips.unshift(skip)
                                    }
                                }
                            });
        ...
        dataGrid.ScrollingDataSourceWrapper = DX.Class.inherit(function() {
            ...
            return {
                    ctor: function(options) {
                        ...
                        var processChanged = function(that, changeType) {
                        ...
                        dataSource.changed.add(function() {
                            ...
                            if (isVirtualMode(that)) {
                                ...
                                if (basePageIndex === dataSource.pageIndex() + 1) {
                                    ...
                                    processChanged(that, that._cache.length > 1 ? 'prepend' : undefined)
                                }
                                else {
                                    that._cache.push({
                                        pageIndex: dataSource.pageIndex(),
                                    ...
                                    processChanged(that, that._cache.length > 1 ? 'append' : undefined)
                                }
                                that.load()
                            }
                            else
    ...
    (function($, DX) {
        ...
        dataGrid.DataSource = DX.data.DataSource.inherit(function() {
            ...
            return {
                    ...
                    totalItemsCount: function() {
                        var that = this;
                        return hasGrouping(that) ? that._groupingHelper.totalItemsCount() : that.totalCount()
                    },
                    itemsCount: function() {
                        var that = this;
                    ...
                    _scheduleChangedCallbacks: function(deferred) {
                        ...
                        deferred.done(function() {
                            ...
                            if (that._totalCount >= 0) {
                                ...
                                itemsLength = that.items().length;
                                currentTotalCount = that.pageIndex() * that.pageSize() + itemsLength;
                                that._currentTotalCount = Math.max(that._currentTotalCount, currentTotalCount);
                                if (itemsLength === 0 && that.pageIndex() >= that.pagesCount()) {
                                    that.pageIndex(that.pagesCount() - 1);
                                    that.load()
                                }
                            }
    ...
    (function($, DX) {
        ...
        dataGrid.StateStoringController = dataGrid.ViewController.inherit(function() {
            var getStorage = function(options) {
                    ...
                    if (!storage)
                        throw new Error(Globalize.localize("dxDataGrid-stateStoringUnsupportedStorageMessage"));
                    return storage
                };
    ...
    (function($, DX, undefined) {
        ...
        var USER_STATE_FIELD_NAMES = ['initialIndex', 'dataField', 'width', 'visible', 'sortOrder', 'sortIndex', 'groupIndex', 'filterText', 'selectedFilterOperation'];
        var ColumnsController = dataGrid.Controller.inherit(function() {
                ...
                var updateColumnParameterIndexes = function(that, indexParameterName) {
                ...
                var updateColumnGroupIndexes = function(that) {
                        var indexPropertyName = 'groupIndex',
                            groupIndex = updateColumnParameterIndexes(that, indexPropertyName);
                        $.each(that._columns, function(index, column) {
                ...
                var updateColumnSortIndexes = function(that) {
                        var sortIndex = updateColumnParameterIndexes(that, 'sortIndex');
                        $.each(that._columns, function(index, column) {
                            if (!isDefined(column.sortIndex) && !isDefined(column.groupIndex) && (column.sortOrder === 'asc' || column.sortOrder === 'desc'))
                ...
                var applyColumnTypesToDataSource = function(that, dataSource) {
                        ...
                        if (dataSource && dataSource.map) {
                            for (i = 0; i < columns.length; i++) {
                                ...
                                if (fieldName && columns[i].dataType === 'date' && (!firstItem || !utils.isDate(firstItem[fieldName])))
                                    dateFieldNames.push(fieldName)
                            }
                            var convertDates = function(item, dateFieldNames, groupsCount) {
                                    if (!groupsCount || !utils.isDefined(item.items))
                                        for (var i = 0; i < dateFieldNames.length; i++) {
                                            var fieldValue = item[dateFieldNames[i]];
                                            if (fieldValue)
                                                item[dateFieldNames[i]] = parseDate(fieldValue)
                                        }
                                    else
                                        $.each(item.items, function(index, item) {
                                            convertDates(item, dateFieldNames, groupsCount - 1)
                                        })
                                };
                            if (dateFieldNames.length > 0) {
                                ...
                                columnTypesMapFunc = function(item) {
                                    ...
                                    if (item)
                                        convertDates(item, dateFieldNames, normalizeSortingInfo(dataSource.group() || []).length);
                                    return item
                                };
                                dataSource.map(columnTypesMapFunc);
                                if (!dataSource.map()) {
                                    dataSource.store().inserting.remove(that._insertingHandler);
                                    that._insertingHandler = function(values) {
                                        columnTypesMapFunc(values)
                                    };
                ...
                var getColumnIndexByVisibleIndex = function(that, visibleIndex, location) {
                        var columns = location === GROUP_LOCATION ? that.getVisibleGroupColumns() : location === COLUMN_CHOOSER_LOCATION ? that.getHiddenColumns() : that.getVisibleColumns(),
                ...
                var updateIndexes = function(that) {
                ...
                return {
                        ...
                        applyDataSource: function(dataSource, forceApplying) {
                            ...
                            if (!that._dataSourceApplied || that._dataSourceColumnsCount === 0 || forceApplying || that.option('regenerateColumnsByVisibleItems'))
                                if (isDataSourceLoaded) {
                                    ...
                                    that.updateColumns(dataSource);
                                    $.when(applyColumnTypesToDataSource(that, dataSource)).always(function() {
                        ...
                        getVisibleGroupColumns: function() {
                            return $.map(this.getGroupColumns(), function(column) {
                                    if (column && column.visible)
                                        return column
                                })
                        },
                        getVisibleColumns: function() {
                            var visibleColumns = $.map(this.getVisibleGroupColumns(), function(column) {
                        ...
                        allowMoveColumn: function(fromVisibleIndex, toVisibleIndex, sourceLocation, targetLocation) {
                            ...
                            if (sourceColumn && (sourceColumn.allowReordering || sourceColumn.allowGrouping || sourceColumn.allowHiding)) {
                                if (sourceLocation === targetLocation) {
                                    if (sourceLocation === COLUMN_CHOOSER_LOCATION && !sourceColumn.allowReordering)
                                        return false;
                        ...
                        moveColumn: function(fromVisibleIndex, toVisibleIndex, sourceLocation, targetLocation) {
                            ...
                            if (fromIndex >= 0) {
                                ...
                                that.columnsChanged.fire({changeType: isGroupMoving ? 'grouping' : 'columns'})
                            }
                        },
                        changeSortOrder: function(columnIndex, sortOrder) {
                            ...
                            if (isSortingChanged) {
                                updateColumnSortIndexes(that);
                                that.columnsChanged.fire({changeType: 'sorting'})
                            }
                        },
                        ...
                        getGroupDataSourceParameters: function() {
                            var group = [];
                            $.each(this.getVisibleGroupColumns(), function() {
                                if (this.dataField)
                                    group.push({
                                        selector: this.dataField,
                                        desc: this.sortOrder === 'desc',
                        ...
                        updateColumns: function(dataSource) {
                            ...
                            if (!dataSource || dataSource.isLoaded()) {
                                sortParameters = dataSource ? normalizeSortingInfo(dataSource.sort() || []) : that.getSortDataSourceParameters();
                                groupParameters = dataSource ? normalizeSortingInfo(dataSource.group() || []) : that.getGroupDataSourceParameters();
                                if (customizeColumns) {
                                ...
                                $.when(that.refresh(true)).always(function() {
                                    that._updateColumnDataTypes(dataSource);
                                    if (!equalSortParameters(sortParameters, that.getSortDataSourceParameters()))
                                        ...
                                        that.columnsChanged.fire({changeType: 'grouping'});
                                    else
                                        that.columnsChanged.fire({changeType: 'columns'})
                                })
                            }
                        },
                        ...
                        columnOption: function(columnIndex, optionName, value) {
                            var that = this,
                            ...
                            if (column) {
                                ...
                                if (utils.isString(optionName)) {
                                    if (arguments.length === 2)
                                        return column[optionName];
                                    if (column[optionName] !== value) {
                                        column[optionName] = value;
                                        isColumnChanged = true
                                    }
                                }
                                else if (utils.isObject(optionName)) {
                                    $.extend(true, column, optionName);
                                    isColumnChanged = true
                                }
                                if (isColumnChanged)
                        ...
                        _createCalculatedColumnOptions: function(columnOptions) {
                            ...
                            if (columnOptions.dataField) {
                                calculatedColumnOptions = {
                                    ...
                                    calculateCellValue: DX.data.utils.compileGetter(columnOptions.dataField),
                                    validate: function(value) {
                                        if (typeof value === 'string')
                                            return !value || !this.parseValue || !!this.parseValue(value);
                                        return true
                                    },
                                    parseValue: function(text) {
                                        ...
                                        if (column.dataType === 'number') {
                                            ...
                                            parsedDate = Globalize.parseDate(text);
                                            if (parsedDate)
                                                result = parsedDate
                                        }
                                        else
                                ...
                                if (columnOptions.lookup)
                                    calculatedColumnOptions.lookup = {
                                        ...
                                        updateValueMap: function() {
                                            ...
                                            this.valueMap = {};
                                            if (this.items) {
                                                calculateValue = DX.data.utils.compileGetter(this.valueExpr);
                                                calculateDisplayValue = DX.data.utils.compileGetter(this.displayExpr);
                                        ...
                                        update: function() {
                                            ...
                                            if (dataSource) {
                                                if (!(dataSource instanceof DX.data.DataSource))
                                                    dataSource = new DX.data.DataSource($.extend(true, {paginate: false}, DX.data.utils.normalizeDataSourceOptions(dataSource)));
    ...
    (function($, DX) {
        ...
        dataGrid.SelectionController = dataGrid.Controller.inherit(function() {
            ...
            var indexOfSelectedItemKey = function(that, key, isSelectAll) {
                    ...
                    if (utils.isObject(key)) {
                        for (index = 0; index < selectedItemKeys.length; index++)
                            if (equalKeys(key, selectedItemKeys[index]))
                                return index;
                        return -1
                    }
                    else
            ...
            var equalKeys = function(key1, key2) {
                    var propertyName,
                        i;
                    if (utils.isObject(key1) && utils.isObject(key2)) {
                        for (propertyName in key1)
                            if (key1.hasOwnProperty(propertyName) && !equalKeys(key1[propertyName], key2[propertyName]))
                                return false;
                        return true
                    }
            ...
            return {
                    ...
                    publicMethods: function() {
                        return ['selectRows', 'getSelectedRowKeys', 'getSelectedRowsData', 'clearSelection', 'selectAll', 'startSelectionWithCheckboxes', 'stopSelectionWithCheckboxes']
                    },
                    ...
                    selectedItemKeys: function(value, isSelectAll) {
                        var that = this,
                            ...
                            isFunctionCriteria,
                            deferred,
                            dataSource = that.getController('data').dataSource(),
                    ...
                    columnHeadersView: {
                        ...
                        _renderHeaderContent: function(rootElement, column, columnIndex) {
                            ...
                            if (column.command === 'select') {
                                ...
                                groupElement = $('<div />').addClass(DATAGRID_CHECKBOX_SIZE_CLASS).dxCheckBox({
                                    value: getSelectAllValue(selectionController),
                                    valueChangeAction: function(e) {
                                        if (getSelectAllValue(selectionController) !== e.value) {
                        ...
                        _getDefaultTemplate: function(column) {
                            ...
                            if (column.command === 'select')
                                return function(container, options) {
                                        ...
                                        $('<div />').addClass(DATAGRID_CHECKBOX_SIZE_CLASS).addClass(DATAGRID_SELECT_CHECKBOX_CLASS).dxCheckBox({
                                            value: options.value,
                                            rtlEnabled: that.option('rtlEnabled')
                                        }).appendTo(container)
                                    };
    ...
    (function($, DX) {
        ...
        var FILTERING_TIMEOUT = 700,
            DATAGRID_CHECKBOX_SIZE = 16,
            DATAGRID_MODULE_NAMESPACE = 'dxDataGridEditorFactory';
        var DATAGRID_FOCUS_OVERLAY_CLASS = 'dx-datagrid-focus-overlay',
            DATAGRID_FOCUSED_ELEMENT_CLASS = 'dx-focused',
            DATAGRID_CONTENT_CLASS = 'dx-datagrid-content',
            EDITOR_CLASS = 'dx-editor';
        dataGrid.EditorFactoryController = dataGrid.Controller.inherit(function() {
            var updateEditorOnResize = function($container, options) {
                    var resizedCallbacks = options.resizedCallbacks,
                        resizedHandler = function(width) {
                            var componentNames = $container.data('dxComponents'),
                                editorName = componentNames && componentNames[0],
                                hasFocus = !!$container.find(':focus').length,
                                editor = editorName && $container.data(editorName);
                            if (editor) {
                                editor._refresh();
                                if (hasFocus)
                                    $container.find('input').focus()
                            }
                            options.resizedCallbacks.remove(resizedHandler)
                        };
                    resizedCallbacks && resizedCallbacks.add(resizedHandler)
                };
            var createCalendarPicker = function($container, options) {
                    ...
                    updateEditorOnResize($container, options)
                };
            var createTextBox = function($container, options) {
                    $container.dxTextBox({
                        ...
                        valueChangeAction: function(e) {
                            ...
                            else if (e.jQueryEvent && e.jQueryEvent.type === 'keydown' && e.jQueryEvent.which === 13)
                                updateValue(e);
                            else
                                updateValue(e)
                        },
                        mode: options.searching ? 'search' : 'text',
                        valueChangeEvent: 'change' + (options.filtering ? ' keyup' : '') + (options.searching ? ' search' : ''),
                        rtlEnabled: options.rtlEnabled,
                        disabled: options.disabled
                    })
                };
            var createBooleanEditor = function($container, options) {
                    if (options.filtering)
                        ...
                        $container.dxCheckBox({
                            height: DATAGRID_CHECKBOX_SIZE,
                            width: DATAGRID_CHECKBOX_SIZE,
            ...
            return {
                    _updateFocusCore: function() {
                        ...
                        if ($dataGridElement) {
                            $focus = $dataGridElement.find(':focus');
                            if ($focus.length) {
                                $focus = $focus.closest('.' + EDITOR_CLASS);
                                if ($focus.length) {
                                    this.focus($focus);
                                    return
                                }
                    ...
                    _updateFocus: function() {
                        setTimeout($.proxy(this._updateFocusCore, this))
                    },
                    ...
                    init: function() {
                        $(document).on(addNamespace('focusout dxpointerdown dxclick', DATAGRID_MODULE_NAMESPACE), this.createAction($.proxy(this._updateFocus, this)))
                    },
                    dispose: function() {
                        $(document).off(addNamespace('focusout dxpointerdown dxclick', DATAGRID_MODULE_NAMESPACE))
                    },
                    createEditor: function($container, options) {
                        var $activeContainer = options.activeContainer,
                            userCreateEditor = this.option('createEditor');
                        if (userCreateEditor && userCreateEditor($container, options))
                            return;
                        ...
                        if ($activeContainer) {
                            $activeContainer.addClass(EDITOR_CLASS);
                            $container.find('.dx-texteditor-input').first().css('text-align', options.alignment)
                        }
                    }
                }
        }());
        ...
        dataGrid.ColumnChooserView = dataGrid.View.inherit({
            ...
            _initializePopupContainer: function() {
                var that = this,
                    ...
                    columnChooserOptions = this.option("columnChooser"),
                    isGenericTheme = DevExpress.ui.themes.current().indexOf("generic") > -1;
    ...
    (function($, DX) {
        ...
        dataGrid.EditingController = dataGrid.ViewController.inherit(function() {
            ...
            return {
                        ...
                        if (!that._saveEditorHandler) {
                            that._saveEditorHandler = that.createAction(function(e) {
                                var event = e.jQueryEvent,
                                    ...
                                    allowEditing,
                                    isDataRow;
                                if (getEditMode(that) === DATAGRID_EDIT_MODE_BATCH)
                                    if (that.isEditing()) {
                                        ...
                                        if (!isDataRow || isDataRow && !allowEditing && !isEditorPopup)
                                            that.closeEditCell()
                                    }
                            });
                    ...
                    optionChanged: function(name, value, prevValue){},
                    publicMethods: function() {
                        return ['insertRow', 'editRow', 'editCell', 'closeEditCell', 'removeRow', 'saveEditData', 'cancelEditData']
                    },
                    refresh: function() {
                    ...
                    isEditRow: function(rowIndex) {
                        return this._editRowIndex === rowIndex
                    },
                    ...
                    processItems: function(items) {
                        var that = this,
                            editKeys = that._editKeys,
                            i,
                            key;
                        that.update();
                        for (i = 0; i < editKeys.length; i++) {
                            key = editKeys[i];
                            if (key && key[DATAGRID_INSERT_KEY] && key.pageIndex === that._pageIndex)
                    ...
                    processDataItem: function(item, columns, generateDataValues) {
                        var that = this,
                            ...
                            editIndex,
                            data,
                            key = item.data[DATAGRID_INSERT_KEY] ? item.data : item.key;
                        editIndex = $.inArray(key, that._editKeys);
                        if (editIndex >= 0) {
                            data = that._editData[editIndex];
                            if (key && key[DATAGRID_INSERT_KEY]) {
                                item.inserted = true;
                                item.key = key;
                                item.data = data
                            }
                    ...
                    insertRow: function() {
                        ...
                        insertKey[DATAGRID_INSERT_KEY] = true;
                        that._editKeys.push(insertKey);
                        that._editData.push({});
                        dataController.updateItems()
                    },
                    ...
                    editCell: function(rowIndex, columnIndex) {
                        var that = this,
                            ...
                            oldRowIndex = that._editRowIndex,
                            dataController = getDataController(that),
                            visibleColumns = that.getController('columns').getVisibleColumns(),
                            column = visibleColumns[columnIndex],
                            item = dataController.items()[rowIndex];
                        if (editing && editing.editEnabled && getEditMode(that) === DATAGRID_EDIT_MODE_BATCH && item && !item.removed && column && column.allowEditing) {
                            that._editRowIndex = rowIndex;
                    ...
                    removeRow: function(rowIndex) {
                        ...
                        if (item) {
                            removeByKey = function(key) {
                                that.refresh();
                                var editIndex = $.inArray(key, that._editKeys);
                                if (editIndex >= 0)
                                    if (key && key[DATAGRID_INSERT_KEY]) {
                                        that._editKeys.splice(editIndex, 1);
                                        that._editData.splice(editIndex, 1)
                                    }
                                    else
                                        that._editData[editIndex] = null;
                                else {
                                    that._editKeys.push(key);
                                    that._editData.push(null)
                                }
                                if (isBatchMode)
                                    dataController.updateItems();
                                else
                    ...
                    _saveEditData: function() {
                        ...
                        if (that._editKeys.length) {
                            $.each(that._editKeys, function(index, key) {
                                var data = that._editData[index],
                                    deferred;
                                if (!data)
                                    deferred = store.remove(key);
                                else if (key && key[DATAGRID_INSERT_KEY])
                    ...
                    updateFieldValue: function(rowKey, dataField, value) {
                        var that = this,
                            editDataIndex;
                        if (rowKey !== undefined) {
                            editDataIndex = $.inArray(rowKey, that._editKeys);
                            if (editDataIndex < 0) {
                                editDataIndex = that._editData.length;
                                that._editData.push({});
                                that._editKeys.push(rowKey)
                            }
                            if (that._editData[editDataIndex])
                                that._editData[editDataIndex][dataField] = value;
                            that._updateEditButtons()
                        }
                    },
                    getColumnTemplate: function(options) {
                        ...
                        if (column.allowEditing && column.dataField && !column.command && that._editRowIndex === options.rowIndex && (that._editColumnIndex < 0 || that._editColumnIndex === options.columnIndex) && options.rowType === 'data') {
                            options.setValue = function(value) {
                                that.updateFieldValue(this.key, this.column.dataField, value)
                            };
                                ...
                                if (that._editRowIndex === options.rowIndex && isRowMode) {
                                    ...
                                    if (editingOptions.removeEnabled)
                                        if (options.row.removed)
                                            createLink(container, editingTexts.recoverRow, 'recoverRow', options);
                                        else
                    ...
                    renderEditButtons: function(rootElement) {
                        ...
                        if (insertButton.length)
                            insertButton.remove();
                        if (editingOptions.editEnabled && getEditMode(that) === DATAGRID_EDIT_MODE_BATCH) {
            ...
            defaultOptions: function() {
                return {editing: {
                            ...
                            texts: {
                                ...
                                recoverRow: Globalize.localize("dxDataGrid-editingRecoverRow"),
            ...
            extenders: {
                controllers: {
                    ...
                    columns: {getVisibleColumns: function() {
                            ...
                            if (editing && (editing.editEnabled && getEditMode(that) === DATAGRID_EDIT_MODE_ROW || editing.removeEnabled || editing.insertEnabled) && columns.length)
                                columns.push({
                                    visible: true,
                                    command: 'edit',
                                    width: 100
                ...
                views: {
                    rowsView: {
                        ...
                        _createRow: function(options) {
                            ...
                            if (options) {
                                ...
                                if (isEditRow || isRowInserted || isRowRemoved || isRowModified) {
                                    $row.removeClass(DATAGRID_ROW_HOVERED);
                        ...
                        _rowClick: function(rowIndex, event) {
                            ...
                            if (cellIndex >= 0 && editingController.editCell(rowIndex, cellIndex))
                                setTimeout(function() {
                                    that._focusEditor()
                                });
                            else if (!editingController.isEditRow(rowIndex))
                                that.callBase(rowIndex, event)
                        },
                        _cellPrepared: function($cell, parameters) {
                            ...
                            parameters.isEditing = this.getController('editing').isEditCell(parameters.rowIndex, columnIndex);
                    ...
                    headerPanel: {
                        ...
                        isVisible: function() {
                            ...
                            return that.callBase() || editingOptions && (editingOptions.insertEnabled || editingOptions.editEnabled && editingOptions.editMode === DATAGRID_EDIT_MODE_BATCH)
                        }
                    }
                }
            }
        });
        $.extend(dataGrid.__internals, {DATAGRID_CELL_MODIFIED: DATAGRID_CELL_MODIFIED})
    })(jQuery, DevExpress);
        ...
        dataGrid.DataController = dataGrid.Controller.inherit({}).include(ui.DataHelperMixin).inherit(function() {
            ...
            return {
                    init: function() {
                        ...
                        that._items = [];
                        that._columnsController = that.getController('columns');
                        that._columnsChangedHandler = function(args) {
                            var updateItemsHandler = function() {
                                    that._columnsController.columnsChanged.remove(updateItemsHandler);
                                    that.updateItems()
                                };
                            if (args.changeType === 'sorting' || args.changeType === 'grouping') {
                                if (that._dataSource) {
                            ...
                            else if (args.changeType === 'columns' && args.optionName && (args.optionName === 'filterText' || args.optionName.filterText !== undefined || (args.optionName === 'selectedFilterOperation' || args.optionName.selectedFilterOperation !== undefined) && args.column.filterText))
                                that._applyFilter();
                            else if (args.optionName !== 'width')
                        ...
                        that._gridDataSourceChangedHandler = function(e) {
                            var dataSource = that._dataSource,
                                ...
                                additionalFilter;
                            if (dataSource) {
                                that._columnsController.applyDataSource(dataSource);
                                isDataSourceApplied = that._columnsController.isDataSourceApplied();
                                additionalFilter = that._calculateAdditionalFilter();
                                if (that._needApplyFilter && isDataSourceApplied && additionalFilter && additionalFilter.length)
                                    that._applyFilter();
                                else
                                    that.updateItems(e);
                                that._needApplyFilter = !isDataSourceApplied
                            }
                        };
                        that._columnsController.columnsChanged.add(that._columnsChangedHandler);
                        that._loadingChangedHandler = function(isLoading) {
                            that._isLoading = isLoading;
                            that.loadingChanged.fire(that.isLoading())
                        };
                    ...
                    publicMethods: function() {
                        return ['beginCustomLoading', 'endCustomLoading', 'refresh', 'filter', 'clearFilter', 'keyOf', 'byKey', 'getDataByKeys', 'collapseAll', 'expandAll', 'pageIndex', '_disposeDataSource']
                    },
                    optionChanged: function(name, value, prevValue) {
                        var that = this;
                        switch (name) {
                    ...
                    _processItems: function(items) {
                        var that = this,
                            visibleColumns = that._columnsController.getVisibleColumns(),
                            groupColumns = that._columnsController.getVisibleGroupColumns(),
                            result = [],
                            dataIndex = 0;
                    ...
                    beginCustomLoading: function(messageText) {
                        ...
                        that.loadingChanged.fire(that.isLoading(), messageText)
                    },
                    ...
                    refresh: function() {
                        ...
                        $.when(this._columnsController.refresh()).always(function() {
                            $.when(dataSource && dataSource.reload()).done(d.resolve).fail(d.reject)
                        });
                        return d
                    },
                    dispose: function() {
    ...
    (function($, DX, undefined) {
        ...
        dataGrid.ColumnsView = dataGrid.View.inherit({
            _applySorting: function(rootElement, column) {
                ...
                if (utils.isDefined(column.sortOrder)) {
                    ...
                    $('<span />').addClass(DATAGRID_SORT_CLASS).css('float', getAlignment(column.alignment)).toggleClass(DATAGRID_SORTUP_CLASS, column.sortOrder === 'asc').toggleClass(DATAGRID_SORTDOWN_CLASS, column.sortOrder === 'desc').prependTo(rootElement)
                }
            },
            ...
            resetColumnWidths: function() {
                ...
                if (that._tableElement) {
                    ...
                    tableElement.css('top', that._tableElement.css('top'));
        ...
        dataGrid.ColumnHeadersView = dataGrid.ColumnsView.inherit({
            _renderHeaderContent: function($cell, column, columnIndex) {
                ...
                if (column.command === "empty" || column.command === "edit")
                    ...
                    that._applySorting($cell, column);
                    var $content = $('<div />').addClass(DATAGRID_CELL_CONTENT_CLASS).appendTo($cell);
                    var templateOptions = {
                            column: column,
                            columnIndex: columnIndex
                        };
            ...
            _renderHeader: function(rootElement, column, columnIndex) {
                var that = this,
                    ...
                    cellElement;
                cellElement = $('<td />').css('text-align', column.alignment).toggleClass(column.cssClass, !utils.isDefined(column.groupIndex));
            ...
            _renderCore: function() {
                ...
                $container.attr("tabIndex", 1).addClass(DATAGRID_HEADERS_CLASS).addClass(DATAGRID_CONTENT_CLASS).toggleClass(DATAGRID_NOWRAP_CLASS, !!that.option("columnAutoWidth") || !that.option("wordWrapEnabled")).empty();
                $('<div/>').addClass(DATAGRID_HEADERS_CONTAINER_CLASS).append(that._tableElement).appendTo($container);
                that._scrollLeft = 0;
                that.setScrollOffset(scrollLeft)
            },
            _afterRender: function($root) {
                var that = this;
                if (!$root)
                ...
                switch (e.optionName) {
                    case'width':
                        ...
                        if (visibleColumnIndex !== -1) {
                            $(this._tableElement.find('col')[visibleColumnIndex]).css('width', e.column.width);
            ...
            allowDragging: function(draggingPanels) {
                var i,
                    result = false;
                if (!draggingPanels)
                    return this.option("allowColumnReordering");
                for (i = 0; i < draggingPanels.length; i++)
                    if (draggingPanels[i].allowDragging())
                        return true;
                return false
            },
            getColumns: function() {
            ...
            setScrollOffset: function(pos) {
                var that = this,
                    $element = that._element();
                if (that._scrollLeft !== pos) {
                    that._scrollLeft = pos;
                    $element && $element.find('.' + DATAGRID_HEADERS_CONTAINER_CLASS).scrollLeft(pos)
                }
            },
    ...
    (function($, DX) {
        ...
        var ColumnHeadersViewFilterRowExtender = {
                _processColumnsChanged: function(e) {
                    var that = this,
                        ...
                        textBox,
                        $menu;
                    if (e.optionName === 'filterText')
                        return;
                    if (e.optionName === 'selectedFilterOperation' || e.optionName && e.optionName.selectedFilterOperation) {
                        ...
                        if (visibleIndex >= 0) {
                            ...
                            if ($menu.length) {
                                that._updateFilterOperationChooser($menu, e.column);
                                textBox = $menu.prev().children().data('dxTextBox');
                                if (textBox)
                                    textBox.option('value', e.column.filterText)
                            }
                        }
                    }
                    else
                    ...
                    if (column.filterOperations && column.filterOperations.length) {
                        result = $.map(column.filterOperations, function(value) {
                            return {
                                    name: value,
                                    selectable: true,
                                    selected: column.selectedFilterOperation ? column.selectedFilterOperation === value : column.filterOperations[0] === value,
                ...
                _renderFilterRowCell: function(rootElement, column) {
                    var that = this,
                        ...
                        $editorContainer,
                        $editor;
                    var updateFilterText = function(that, column, value) {
                            var filterOptions,
                                filterText = DX.formatHelper.format(value, column.format, column.precision),
                                columnsController = that._columnsController;
                            if (!column.filterText && !filterText)
                                return;
                            column.filterText = filterText;
                            columnsController.columnOption(column.index, 'filterText', filterText)
                        };
                    if (column.command === 'empty')
                        ...
                        that.getController('editorFactory').createEditor($editor, $.extend({}, column, {
                            activeContainer: $cell,
                            value: function(value) {
                                if (value === undefined)
                                    return column.parseValue ? column.parseValue(column.filterText) : column.filterText;
                                else
                                    updateFilterText(that, column, value)
                            },
                            filtering: true,
        ...
        var processFilterExpession = function(text) {
                var operation,
                    i,
                    compareOperations = ['>=', '<=', '<>', '=', '>', '<'];
                if (!text)
                    return null;
                for (i = 0; i < compareOperations.length; i++)
                    if (text.indexOf && text.indexOf(compareOperations[i]) === 0) {
                        operation = compareOperations[i];
                        text = text.substr(operation.length);
                        break
                    }
                if (!text)
                    return null;
        ...
        dataGrid.processFilterExpession = processFilterExpession;
        var DataControllerFilterRowExtender = {_calculateAdditionalFilter: function() {
                    ...
                    $.each(columns, function() {
                        var filterOptions,
                            filter;
                        if (this.allowFiltering && this.calculateFilterExpression && this.filterText) {
                            if (this.selectedFilterOperation)
                                filter = this.calculateFilterExpression(this.filterText, this.selectedFilterOperation);
                            else {
                                filterOptions = processFilterExpession(this.filterText);
                                if (filterOptions)
                                    filter = this.calculateFilterExpression(filterOptions.value, filterOptions.operation)
                            }
                            if (filter)
                                filters.push(filter)
                        }
        ...
        var ColumnsControllerFilterRowExtender = {_createCalculatedColumnOptions: function(columnOptions) {
                    ...
                    if (columnOptions.dataField) {
                        $.extend(calculatedColumnOptions, {
                            calculateFilterExpression: function(text, selectedFilterOperation) {
                                var column = this,
                                    filter = null,
                                    parsedValue = column.parseValue(text);
                                if (parsedValue !== undefined)
                                    if (column.dataType === 'string')
                                        filter = [column.dataField, selectedFilterOperation || 'contains', parsedValue];
                                    else
                                        filter = [column.dataField, selectedFilterOperation || '=', parsedValue];
                                return filter
                            },
                            allowFiltering: true
                        });
                        if (columnOptions.displayMap)
                            $.extend(calculatedColumnOptions, {
                                dataType: 'string',
                                customizeText: function(options) {
                                    var value = options.value,
                                        displayText = value !== undefined ? columnOptions.displayMap[value] : undefined;
                                    return displayText !== undefined ? displayText : columnOptions.displayMap['default']
                                },
                                calculateFilterExpression: function(text, selectedFilterOperation) {
                                    var key,
                                        value;
                                    for (key in this.displayMap) {
                                        value = this.displayMap[key];
                                        if (key !== 'default' && value && (selectedFilterOperation ? value === text : value.indexOf && value.indexOf(text) !== -1)) {
                                            if (utils.isNumber(key))
                                                key = new Number(key);
                                            return [this.dataField, selectedFilterOperation || '=', key]
                                        }
                                    }
                                    return null
                                }
        ...
        dataGrid.HeaderPanel = dataGrid.ColumnsView.inherit({
            _renderGroupPanel: function() {
                var that = this,
                    ...
                    $groupPanel,
                    groupColumns = that.getController('columns').getVisibleGroupColumns();
            ...
            allowDragging: function() {
                ...
                return groupPanelOptions && groupPanelOptions.visible && groupPanelOptions.allowColumnDragging
            },
            getColumnElements: function() {
                var $element = this._element();
            ...
            getColumns: function() {
                return this.getController('columns').getVisibleGroupColumns()
            },
            getBoundingRect: function() {
                var that = this,
                    $element = that._element(),
                        ...
                        var calculateSearchFilter = function(that, text) {
                                ...
                                for (i = 0; i < columns.length; i++) {
                                    ...
                                    if (column.allowFiltering && column.calculateFilterExpression) {
                                        filter = column.calculateFilterExpression(text);
                                        if (filter) {
                                ...
                                _renderSearchPanel: function() {
                                    ...
                                    if (searchPanelOptions && searchPanelOptions.visible) {
                                        ...
                                        that.getController('editorFactory').createEditor(that._$searchPanel, {
                                            ...
                                            value: function(value) {
                                                if (value === undefined)
                                                    return dataController.getSearchText();
                                                else
    ...
    (function($, DX) {
        ...
        dataGrid.RowsView = dataGrid.ColumnsView.inherit({
            ...
            _getDefaultGroupTemplate: function() {
                var scrollingMode = this.option('scrolling.mode'),
                    ...
                    align = this.option('rtlEnabled') ? 'right' : 'left';
                return function(container, options) {
                        var data = options.data,
                            text = options.column.caption + ": " + options.text;
                        if (scrollingMode !== 'virtual' && scrollingMode !== 'infinite' && data && grouping)
                            if (data.isContinuationOnNextPage && data.isContinuation)
                                text += ' (' + grouping.groupContinuedMessage + '. ' + grouping.groupContinuesMessage + ')';
                            else if (data.isContinuationOnNextPage)
                                text += ' (' + grouping.groupContinuesMessage + ')';
                            else if (data.isContinuation)
                                text += ' (' + grouping.groupContinuedMessage + ')';
                        container.css('text-align', align).text(text)
                    }
            },
            _update: function(change){},
            _createRow: function(rowOptions) {
                ...
                if (rowOptions) {
                    ...
                    $row.toggleClass(DATAGRID_DATA_ROW_CLASS, isDataRow).toggleClass(DATAGRID_ROW_ALTERNATION_CLASS, isDataRow && rowOptions.dataIndex % 2 === 1 && this.option('rowAlternationEnabled')).toggleClass(DATAGRID_ROW_LINES_CLASS, isDataRow && this.option('showRowLines')).toggleClass(DATAGRID_COLUMN_LINES_CLASS, this.option('showColumnLines')).toggleClass(DATAGRID_GROUP_ROW_CLASS, isGroup).toggleClass(DATAGRID_ROW_HOVERED, !!this.option("hoverStateEnabled") && !isGroup)
                }
                return $row
            },
            ...
            _createCell: function(value, item, rowIndex, column, columnIndex) {
                ...
                $cell[0].style.textAlign = column.alignment || '';
                that._renderTemplate($cell, template, parameters);
            ...
            _renderScrollable: function($table) {
                ...
                if (!utils.isDefined(that._tableElement)) {
                    ...
                    if ($element.dxLoadPanel) {
                        ...
                        if (loadPanelOptions && loadPanelOptions.enabled) {
                            var loadPanelOptions = {
                                    ...
                                    width: loadPanelOptions.width,
                                    height: loadPanelOptions.height,
                                    animation: loadPanelOptions.animation,
                                    position: {of: $element},
                    ...
                    if (!that._windowScrollHandler) {
                        that._windowScrollHandler = that.createAction(function() {
                            var contentOffset,
                                scrollTop;
                            if (!that._hasHeight && that._rowHeight) {
                                contentOffset = $element.offset().top;
                                scrollTop = $(window).scrollTop() - contentOffset;
                                scrollTop = scrollTop > 0 ? scrollTop : 0;
                                that._scrollTop = scrollTop;
                                that._dataController.setViewportItemIndex(scrollTop / that._rowHeight)
                            }
                        });
                        $(window).on('scroll', that._windowScrollHandler);
                        disposing && disposing.add(function() {
                            $(window).off('scroll', that._windowScrollHandler)
                        })
                    }
                }
            },
            _renderContent: function(contentElement, tableElement) {
                ...
                if (virtualItemsCount) {
                    ...
                    tableElement.css('top', virtualItemsCount.begin * rowHeight);
                    ...
                    contentHeight = (virtualItemsCount.begin + virtualItemsCount.end + that._dataController.items().length) * rowHeight;
                    if (that._contentHeight !== contentHeight) {
            ...
            _updateContent: function(tableElement, changeType) {
                ...
                if (changeType === 'append' || changeType === 'prepend') {
                    ...
                    if (virtualItemsCount) {
                        tableElement.addClass(DATAGRID_TABLE_CONTENT_CLASS);
                        contentTable.css('top', virtualItemsCount.begin * rowHeight);
                        tableElement.find('tr')[changeType === 'append' ? 'appendTo' : 'prependTo'](contentTable)
                    }
                    else
                        tableElement[changeType === 'append' ? 'insertAfter' : 'insertBefore'](contentTable)
                }
                else
                    that._renderContent(contentElement, tableElement);
                that._updateBorderLastRow()
            },
            ...
            _updateRowHeight: function() {
                var that = this,
                    ...
                    viewportHeight;
                if ($element && itemsCount > 0 && !that._rowHeight) {
                    tableHeight = that._tableElement.height();
                    if (that._freeSpaceRowElement && that._freeSpaceRowElement.is(':visible'))
                        tableHeight -= that._freeSpaceRowElement.height();
                    that._rowHeight = tableHeight / itemsCount;
                    if (that._rowHeight) {
                        that._updateContent(that._tableElement);
                        viewportHeight = that._hasHeight ? $element.height() : $(window).height();
            ...
            _updateContainerHeight: function(container, height) {
                var that = this,
                    columns = that._columnsController.getVisibleColumns(),
                    html = dataGrid.createColGroup(columns).wrapAll('<div />').parent().html(),
                    freeSpaceCellsHtml = '',
                    i,
            ...
            _updateBorderLastRow: function() {
                var that = this,
                    rowItems = that._getRowElements(),
                    isPagerVisible = that.getView('pagerView') && that.getView('pagerView').isVisible(),
                    tableElement = that._tableElement;
                that._element().find('.' + DATAGRID_LAST_ROW).removeClass(DATAGRID_LAST_ROW);
                var $lastFreespaceRow = tableElement.find('.' + DATAGRID_FREESPACE_CLASS).last();
                if (isPagerVisible && rowItems.length && $lastFreespaceRow.height() === 0)
                    $lastFreespaceRow.prev().addClass(DATAGRID_LAST_ROW)
            },
            _appendRow: function(tableElement, rowElement) {
                var that = this;
                if (that.option("rowTemplate") && that._delayedTemplates.length)
                    that._delayedTemplates.push({
            ...
            _renderTable: function(items) {
                ...
                for (i = 0; i < itemsLength; i++) {
                    ...
                    if (item.groupIndex !== undefined) {
                        ...
                        if (columns[groupEmptyCellsCount].allowCollapsing)
            ...
            _renderCore: function(change) {
                var that = this,
                    ...
                    $element = that._element(),
                    $root = $element.parent();
                $element.attr("tabIndex", 0).addClass(DATAGRID_ROWS_VIEW_CLASS).toggleClass(DATAGRID_NOWRAP_CLASS, !that.option("wordWrapEnabled"));
            ...
            updateFreeSpaceRowHeight: function() {
                ...
                if (that._freeSpaceRowElement && contentElement) {
                    ...
                    $.each(contentElement.children(), function() {
                    ...
                    if (that._dataController.items().length > 0)
                        if (resultHeight > 0) {
                            ...
                            var freespaceRowCount = that._dataController.pageSize() - that._dataController.items().length;
                            if (freespaceRowCount > 0 && that._dataController.getPagesCount() > 1) {
            ...
            init: function() {
                ...
                that._columnsController.columnsChanged.add(function(e) {
                    ...
                    if (e.optionName === 'width') {
                        ...
                        if (visibleColumnIndex !== -1) {
                            that._tableElement.find('col').eq(visibleColumnIndex).css('width', e.column.width);
                            if (e.column.resizedCallbacks)
                                e.column.resizedCallbacks.fire(e.column.width);
            ...
            renderDelayedTemplates: function() {
                ...
                while (delayedTemplates.length) {
                    ...
                    this._highlightSearchText(templateParameters.container)
                }
            },
            ...
            setLoading: function(isLoading, messageText) {
                var that = this,
                    ...
                    loadPanelOptions = that.option("loadPanel");
                if (loadPanel) {
                    loadPanel.option('message', messageText || loadPanelOptions.text);
                    loadPanel.option('visible', hasBottomLoadPanel ? false : isLoading)
                }
            },
        ...
        $.extend(dataGrid.__internals, {
            ...
            DATAGRID_ROW_HOVERED: DATAGRID_ROW_HOVERED,
        ...
        dataGrid.PagerView = dataGrid.View.inherit({
            ...
            _renderCore: function() {
                ...
                $element.dxPager({
                    ...
                    pagesCount: dataController.getPagesCount(),
                    pageSize: dataController.pageSize(),
            ...
            getPageSizes: function() {
                var that = this,
                    ...
                    pageSize = dataController.pageSize();
                if (!utils.isDefined(that._pageSizes)) {
                    that._pageSizes = [];
                    if (pagerOptions)
            ...
            isVisible: function() {
                ...
                if (pagerVisible === 'auto')
                    if (scrolling && (scrolling.mode === 'virtual' || scrolling.mode === 'infinite'))
                        ...
                        pagerVisible = dataController.getPagesCount() > 1 || dataController.isLoaded() && !dataController.hasKnownLastPage();
                that._isVisible = pagerVisible;
                return pagerVisible
            },
            getHeight: function() {
                var pager = this._getPager();
                return pager ? pager.getHeight() : 0
            },
            optionChanged: function(name, value, prevValue) {
                var that = this;
                switch (name) {
    ...
    (function($, DX) {
        ...
        var DATAGRID_CLASS = "dx-datagrid",
            ...
            LOADPANEL_MARGIN = 50,
            VIEW_NAMES = ['columnsSeparatorView', 'blockSeparatorView', 'trackerView', 'headerPanel', 'columnHeadersView', 'rowsView', 'columnChooserView', 'pagerView', 'draggingHeaderView', 'errorView'];
        ...
        dataGrid.GridView = dataGrid.View.inherit({
            _initializeResizeHandlers: function() {
                var that = this;
            ...
            _getTotalWidth: function(widths, groupWidth) {
                ...
                for (i = 0; i < widths.length; i++) {
                    width = widths[i];
                    if (width)
                        result += utils.isString(width) && width.slice(-1) === '%' ? Math.round(parseInt(width) * groupWidth / 100) : parseInt(width)
                }
                return result
            },
            getView: function(name) {
            ...
            resize: function() {
                ...
                if (that._hasHeight && rootElementHeight > 0 && that.option('scrolling'))
                    ...
                    that._rowsView.height(loadPanelOptions && loadPanelOptions.visible ? loadPanelOptions.height + LOADPANEL_MARGIN : EMPTY_GRID_ROWS_HEIGHT);
                else
            ...
            init: function() {
                ...
                that._rowsView.scrollOffsetChanged.add(function(e) {
                    that._columnHeadersView.setScrollOffset(e.left)
                });
                that._options = {}
            },
            element: function() {
            ...
            render: function(rootElement) {
                ...
                that._initializeResizeHandlers();
                that.resize();
                that._rowsView.setLoading(that._dataController.isLoading())
            }
        });
    ...
    (function($, DX) {
        ...
        var DATAGRID_COLUMNS_SEPARATOR_CLASS = "dx-datagrid-columns-separator",
            ...
            DRAGGING_DELTA = 5;
        var allowDropColumn = function(sourceColumn, targetColumn, location) {
                switch (location) {
                    case"headers":
                        return sourceColumn.allowReordering && targetColumn.allowReordering;
                    case"group":
        ...
        dataGrid.TrackerView = dataGrid.View.inherit({
            ...
            isVisible: function() {
                ...
                return columnsController.getCommonSettings().allowResizing
            },
            show: function() {
                this._element().show()
            },
        ...
        dataGrid.ColumnsSeparatorView = dataGrid.View.inherit({
            ...
            isVisible: function() {
                var commonColumnSettings = this.getController("columns").getCommonSettings();
                return this.option('showColumnHeaders') && (commonColumnSettings.allowReordering || commonColumnSettings.allowResizing)
            },
            init: function() {
                this.callBase();
                var commonColumnSettings = this.getController("columns").getCommonSettings();
                this._isTransparent = commonColumnSettings.allowResizing;
                if (this.isVisible())
            ...
            moveByX: function(outerX) {
                var $element = this._element();
                if ($element) {
                    this._element().css("left", outerX - this._$root.offset().left);
        ...
        dataGrid.BlockSeparatorView = dataGrid.ColumnsSeparatorView.inherit({
            ...
            hide: function() {
                var that = this;
                that.callBase();
                if (that._$root && !that._$root.children('.' + DATAGRID_BLOCK_SEPARATOR_CLASS).length)
                    that._$root.prepend(that._element())
            },
            isVisible: function() {
                var groupPanelOptions = this.option("groupPanel"),
                    columnChooserOptions = this.option('columnChooser');
        ...
        dataGrid.DraggingHeaderView = dataGrid.View.inherit({
            ...
            _dropHeader: function(args) {
                ...
                that._element().appendTo(that._$root);
                ...
                if (that._isDragging) {
                    ...
                    if (that._element().is(':visible') || moveDeltaX > DRAGGING_DELTA || moveDeltaY > DRAGGING_DELTA) {
                        ...
                        if (targetDraggingPanel) {
                            ...
                            commonColumnSettings = that.getController("columns").getCommonSettings();
            ...
            _renderCore: function() {
                this._element().addClass(DATAGRID_DRAGGING_HEADER_CLASS + ' ' + DATAGRID_CELL_CONTENT_CLASS).css('display', 'none')
            },
            _afterRender: function($root) {
                this._$root = $root;
                this._unsubscribeFromEvents();
                this._subscribeToEvents($root)
            },
            ...
            isVisible: function() {
                ...
                return this.option('showColumnHeaders') && (commonColumnSettings.allowReordering || commonColumnSettings.allowGrouping || commonColumnSettings.allowHiding)
            },
            init: function() {
                this.callBase();
        ...
        dataGrid.ColumnsResizerViewController = dataGrid.ViewController.inherit({
                ...
                if (that._isResizing) {
                    if (parentOffsetLeft <= eventData.x && eventData.x <= parentOffsetLeft + that._$parentContainer.width())
                        if (that._updateColumnsWidthIfNeeded(that._targetPoint.columnIndex, eventData.x)) {
                            that._columnsSeparatorView.moveByX(eventData.x - deltaX);
            ...
            _unsubscribeFromEvents: function() {
                this._$parentContainer.off(addNamespace('dxpointermove', DATAGRID_MODULE_NAMESPACE));
                this._columnsSeparatorView._element().off(addNamespace('dxpointerdown', DATAGRID_MODULE_NAMESPACE));
                this._columnsSeparatorView._element().off(addNamespace('dxpointerup', DATAGRID_MODULE_NAMESPACE));
                $(document).off(addNamespace('dxpointerup', DATAGRID_MODULE_NAMESPACE))
            },
            _subscribeToEvents: function() {
                this._$parentContainer.on(addNamespace('dxpointermove', DATAGRID_MODULE_NAMESPACE), this, this.createAction(this._moveSeparator, {excludeValidators: ['gesture']}));
                this._columnsSeparatorView._element().on(addNamespace('dxpointerdown', DATAGRID_MODULE_NAMESPACE), this, this.createAction(this._startResizing));
                this._columnsSeparatorView._element().on(addNamespace('dxpointerup', DATAGRID_MODULE_NAMESPACE), this, this.createAction(this._endResizing, {excludeValidators: ['gesture']}));
                $(document).on(addNamespace('dxpointerup', DATAGRID_MODULE_NAMESPACE), this, this.createAction(this._endResizing, {excludeValidators: ['gesture']}))
            },
            _updateColumnsWidthIfNeeded: function(columnIndex, posX) {
                ...
                if (isUpdated) {
                    visibleColumns[columnIndex] && this._columnsController.columnOption(visibleColumns[columnIndex].index, 'width', Math.floor(cellWidth));
                    visibleColumns[columnIndex + 1] && this._columnsController.columnOption(visibleColumns[columnIndex + 1].index, 'width', Math.floor(nextCellWidth))
                }
                return isUpdated
            },
            ...
            init: function() {
                ...
                that.callBase();
                if (that.getController('columns').getCommonSettings().allowResizing) {
        ...
        dataGrid.DraggingHeaderViewController = dataGrid.ViewController.inherit({
            _subscribeToEvent: function(draggingHeader, draggingPanels) {
                ...
                $.each(draggingPanels, function(_, draggingPanel) {
                    if (draggingPanel) {
                        var allowPanelDragging = draggingPanel.allowDragging(draggingPanels),
                            columnElements = draggingPanel.getColumnElements() || [],
                        ...
                        $.each(columnElements, function(index, columnElement) {
                            ...
                            if (allowPanelDragging && that.allowDragColumn(column, nameDraggingPanel))
            ...
            allowDragColumn: function(column, namePanel) {
                return column && (namePanel === "headers" && (column.allowReordering || column.allowGrouping || column.allowHiding) || namePanel === "group" && column.allowGrouping || namePanel === "columnChooser" && column.allowHiding)
            },
            dock: function(parameters) {
                var that = this,
                    ...
                    commonColumnSettings = that._columnsController.getCommonSettings();
    ...
    /*! Module widgets-web, file ui.dataGrid.contextMenuController.js */
    (function($, DX) {
        var ui = DX.ui,
            utils = DX.utils,
            dataGrid = ui.dataGrid;
        var DATAGRID_CLASS = "dx-datagrid";
        dataGrid.ContextMenuViewController = dataGrid.ViewController.inherit({
            viewNames: function() {
                return ["columnHeadersView"]
            },
            init: function() {
                var that = this,
                    $container = that.component._element();
        ...
        dataGrid.registerModule("contextMenu", {controllers: {contextMenu: dataGrid.ContextMenuViewController}})
    })(jQuery, DevExpress);
        ...
        dataGrid.KeyboardNavigationController = dataGrid.ViewController.inherit({
            ...
            _initFocusedViews: function() {
                var that = this,
                    $cell,
                    $input,
                    blurHandler = that.createAction(function() {
                    ...
                    clickHandler = function(e) {
                        ...
                        if (!that._editingController.isEditing()) {
                            ...
                            that._updateFocusedCellPosition($(event.currentTarget))
                        }
                    };
                that._focusedViews = [];
                    ...
                    if (view)
                        view.renderCompleted.add(function() {
                            ...
                            $element.on(events.addNamespace("dxpointerdown", "dxDataGridKeyboardNavigation"), "." + DATAGRID_ROW_CLASS + " td", {
                                viewIndex: index,
                                view: view
                            }, that.createAction(clickHandler));
                            $element.off(events.addNamespace("blur", "dxDataGridKeyboardNavigation"), blurHandler);
                            $element.on(events.addNamespace("blur", "dxDataGridKeyboardNavigation"), blurHandler);
                            that._initKeyDownProcessor(that, $element, that._keyDownHandler);
                            if (that._focusedView && that._focusedView.name === view.name) {
            ...
            _keyDownHandler: function(e) {
                var editingOptions = this.option("editing"),
                    ...
                    pagesCount = this._dataController.getPagesCount(),
                ...
                switch (e.key) {
                    ...
                    case"space":
                        ...
                        break;
                    case"A":
                        if (e.ctrl && this.option("selection") && this.option("selection").mode === "multiple")
