...
if (!DevExpress.MOD_VIZ_CORE) {
    ...
    (function($, DX, undefined) {
        ...
        core.tickProvider = {
            ...
            isOverlappedTicks: function(ticks, options) {
                ...
                return that._getAutoArrangementStep(ticks, options) > 1
            },
    ...
    (function(DX, undefined) {
        ...
        DX.viz.core.Translator1D = DX.Class.inherit({
            ...
            setup: function(settings) {
                var that = this;
                settings = settings || {};
                that._domainStart = Number(settings.domainStart);
                that._domainEnd = Number(settings.domainEnd);
                that._codomainStart = Number(settings.codomainStart);
                that._codomainEnd = Number(settings.codomainEnd);
                that._domainDelta = that._domainEnd - that._domainStart;
                that._codomainDelta = that._codomainEnd - that._codomainStart;
                return that
            },
            ...
            translate: function(value) {
                var that = this,
                    ratio = (value - that._domainStart) / that._domainDelta;
                return 0 <= ratio && ratio <= 1 ? that._codomainStart + ratio * that._codomainDelta : NaN
            },
            adjust: function(value) {
                var that = this,
                    ...
                    result = NaN;
                if (ratio < 0)
                    result = that._domainStart;
                else if (ratio > 1)
                    result = that._domainEnd;
        ...
        $.extend(core.Translator2D.prototype, {
            reinit: function() {
                ...
                switch (range.axisType) {
                    ...
                    case"discrete":
                        ...
                        canvasOptions.interval = canvasOptions.canvasLength / (categories.length - (range.stick ? 1 : 0));
            ...
            _prepareCanvasOptions: function() {
                var that = this,
                    ...
                    canvasOptions = that._canvasOptions = {},
                    length;
                canvasOptions.rangeMin = businessRange.min;
                canvasOptions.rangeMax = businessRange.max;
                canvasOptions.rangeMinVisible = businessRange.minVisible;
                canvasOptions.rangeMaxVisible = businessRange.maxVisible;
                ...
                if (businessRange.axisType === "logarithmic") {
                    canvasOptions.base = businessRange.base;
                    canvasOptions.rangeMaxVisible = getLog(canvasOptions.rangeMaxVisible, canvasOptions.base);
                    canvasOptions.rangeMinVisible = getLog(canvasOptions.rangeMinVisible, canvasOptions.base);
                    canvasOptions.rangeMin = getLog(canvasOptions.rangeMin, canvasOptions.base);
                    canvasOptions.rangeMax = getLog(canvasOptions.rangeMax, canvasOptions.base)
                }
    ...
    (function(DX, $, undefined) {
        ...
        DX.viz.core.Legend = DX.Class.inherit({
            ...
            releaseHover: function(id) {
                this._reset(id);
                return this
            },
            releaseSelected: function(id) {
                this._reset(id);
                return this
            },
            _reset: function(id) {
            ...
            getBoundingRect: function() {
                return this._insideLegendGroup ? this.boundingRect : {
                        width: 0,
                        height: 0,
                        x: 0,
                        y: 0
            ...
            getLayoutOptions: function() {
                ...
                if (options) {
                    var boundingRect = this.getBoundingRect(),
                        bbox = this._legendGroup.getBBox();
                    return {
                            width: boundingRect.width,
                            height: boundingRect.height,
                            _width: bbox.width,
                            _height: bbox.height,
                            verticalIndent: options.verticalIndent || 0,
                            horizontalIndent: options.horizontalIndent || 0,
                            verticalAlignment: options.verticalAlignment,
                            horizontalAlignment: options.horizontalAlignment,
                            visible: options.visible,
                            orientation: options.orientation,
                            position: options.position,
                            cutLayoutSide: options.orientation === 'horizontal' ? options.verticalAlignment : options.horizontalAlignment
                        }
                }
                return null
            },
            ...
            locate: function(x, y) {
                var that = this,
                    translation = {
                        translateX: x + that._options.paddingLeftRight,
                        translateY: y + that._options.paddingTopBottom
                    };
                that._insideLegendGroup && that._insideLegendGroup.applySettings(translation);
                that._trackerGroup && that._trackerGroup.applySettings(translation);
                return this
            },
            ...
            applyEqualLimitsMargins: function() {
                ...
                if (isDefinedUtils(that[minSelector]) && isDefinedUtils(that[maxSelector]) && that[minSelector].valueOf() === that[maxSelector].valueOf()) {
                    ...
                    if (isLogarithmic) {
                        ...
                        that[minSelector] = that[minSelector] !== 0 ? newMin : 0;
        ...
        viz.core.series.Series = DX.Class.inherit({
            ...
            _setSelectedState: function(state, mode, legendCallback) {
                ...
                if (state && !this.isSelected()) {
                    ...
                    this.fullState = this.fullState & ~SELECTED_STATE;
                    this._changeStyle(legendCallback, "releaseSelected")
                }
            },
            _setHoverState: function(state, mode, legendCallback) {
                ...
                if (state && !this.isHovered()) {
                    ...
                    this.fullState = this.fullState & ~HOVER_STATE;
                    !this.isSelected() && this._changeStyle(legendCallback, "releaseHover")
                }
            },
            ...
            _calcStyle: function() {
                ...
                switch (this.fullState & 3) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        isHoverIncludeModeAndSeriesExcludeMode = this.lastSelectionMode === EXLUDE_POINTS && this.lastHoverMode === INCLUDE_POINTS;
                        result = {
            ...
            _customizePoint: function(pointData) {
                ...
                if (customizeLabel && customizeLabel.call) {
                    ...
                    if (customLabelOptions && !_isEmptyObject(customLabelOptions)) {
                        customLabelOptions = _extend(true, {}, options.label, customLabelOptions);
                ...
                if (customizePoint && customizePoint.call) {
                    ...
                    if (customOptions && !_isEmptyObject(customOptions))
            ...
            releasePointHoverState: function(point, legendCallback) {
                ...
                if (!(this.isSelected() && (this.lastSelectionMode === ALL_SERIES_POINTS_MODE || this.lastSelectionMode === INCLUDE_POINTS)) && !point.isSelected())
                    if (!(this.isHovered() && (this.lastHoverMode === ALL_SERIES_POINTS_MODE || this.lastHoverMode === INCLUDE_POINTS))) {
                        ...
                        legendCallback && legendCallback("releaseHover")
                    }
            },
    ...
    (function($, DX) {
        ...
        series.mixins.chart.scatter = {
            ...
            updateTeamplateFieldNames: function() {
                var that = this,
                    options = that._options;
                options.valueField = options.valueField + that.name;
                options.tagField = options.tagField + that.name
            },
    ...
    (function($, DX) {
        ...
        var baseRangeSeries = {
                ...
                updateTeamplateFieldNames: function() {
                    var that = this,
                        options = that._options;
                    options.rangeValue1Field = options.rangeValue1Field + that.name;
                    options.rangeValue2Field = options.rangeValue2Field + that.name;
                    options.tagField = options.tagField + that.name
                },
                _processRange: function(point, prevPoint) {
        ...
        series.bubble = _extend({}, scatterSeries, {
            ...
            updateTeamplateFieldNames: function() {
                var that = this,
                    options = that._options,
                    name = that.name;
                options.valueField = options.valueField + name;
                options.sizeField = options.sizeField + name;
                options.tagField = options.tagField + name
            },
            _clearingAnimation: function(translators, drawComplete) {
                var that = this,
                    settings = {r: 0},
        ...
        series.stock = _extend({}, scatterSeries, {
            ...
            updateTeamplateFieldNames: function() {
                var that = this,
                    options = that._options,
                    name = that.name;
                options.openValueField = options.openValueField + name;
                options.highValueField = options.highValueField + name;
                options.lowValueField = options.lowValueField + name;
                options.closeValueField = options.closeValueField + name;
                options.tagField = options.tagField + name
            },
    ...
    (function($, DX) {
        ...
        DX.viz.core.series.points.Label = DX.Class.inherit({
            ...
            setReductionColor: function(reductionColor) {
                this._reductionColor = reductionColor
            },
            hide: function() {
            ...
            _drawBackground: function(renderer) {
                var that = this,
                    ...
                    settings;
                if (that._reductionColor)
                    background = _extend({}, background, {fill: that._reductionColor});
            ...
            _drawConnector: function(renderer) {
                var that = this,
                    ...
                    points;
                if (that._reductionColor)
                    connector = _extend({}, connector, {stroke: that._reductionColor});
            ...
            _setPositionForBar: function() {
                var that = this,
                    offset = that._offset,
                    labelPosition = that._options.position || "outside",
                    labelBbox = that._insideGroup.getBBox(),
                    coords;
            ...
            _setPositionForBubble: function() {
                var that = this,
                    ...
                    x = 0,
                    y = 0;
                if (that._options.position === "outside")
    ...
    (function($, DX) {
        ...
        series.points.mixins.symbolPoint = {
            ...
            _updateLabelOptions: function(type) {
                ...
                this._label.updateOptions(this._getLabelOptions(type))
            },
            ...
            _getCrossMarkerCoords: function(radius) {
                var floorHalfRadius = _floor(radius / 2),
                    ceilHalfRadius = _ceil(radius / 2);
                return [{
                            x: -radius,
                            y: -floorHalfRadius
                        }, {
            ...
            _updateMarker: function(animationEnabled, style) {
                ...
                if (that._checkImage(image))
                    settings = that._getImageSettings(image);
                else
    ...
    (function($, DX) {
        ...
        points.piePoint = _extend({}, points.symbolPoint, {
            ...
            _changeData: function(data) {
                ...
                that.tag = data.tag;
                that.radiusInner = DEFAULT_RADIUS_INNER;
                that.radiusOuter = DEFAULT_RADIUS_OUTER;
                that.centerX = DEFAULT_CENTER_X;
                that.centerY = DEFAULT_CENTER_Y;
                that._visible = true;
            ...
            _updateLabelOptions: function() {
                !this._label && this._createLabel();
                this._label.updateOptions(this._getLabelOptions())
            },
            _drawLabel: function(renderer, group) {
    ...
    (function($, DX) {
        ...
        points.rangeSymbolPoint = _extend({}, points.symbolPoint, {
            ...
            _updateLabelOptions: function(type) {
                ...
                this._topLabel.updateOptions(options);
                this._bottomLabel.updateOptions(options)
            },
            setAdjustSeriesLabels: function(adjustSeriesLabels) {
        ...
        points.candlestickPoint = _extend({}, points.barPoint, {
            ...
            _drawLabel: function(renderer, group) {
                ...
                if (this.hasValue() && (commonVisibility || customVisibility)) {
                    ...
                    this._isReduction && this._label.setReductionColor(this._options.reduction.color);
    ...
    (function($, DX, undefined) {
        ...
        viz.charts.ThemeManager = viz.core.BaseThemeManager.inherit(function() {
            ...
            var processAxisOptions = function(axisOptions, name, mergeTitleOptions) {
                    if (!axisOptions)
                        return;
                    axisOptions = $.extend(true, {}, axisOptions);
                    axisOptions.title = processTitleOptions(axisOptions.title);
                    mergeTitleOptions[name] && (axisOptions.title = $.extend(true, mergeTitleOptions[name], axisOptions.title));
                    mergeTitleOptions[name] = axisOptions.title;
            ...
            var applyParticularAxisOptions = function(name, userOptions, rotated) {
                    var theme = this._theme,
                        ...
                        commonAxisSettings = processAxisOptions(this._userOptions["commonAxisSettings"], name, this._mergeAxisTitleOptions);
                    return $.extend(true, {}, theme.commonAxisSettings, theme[position], theme[name + "Style"], commonAxisSettings, processAxisOptions(userOptions, name, this._mergeAxisTitleOptions))
                };
            ...
            var applyParticularTheme = {
                    ...
                    title: function(name) {
                        ...
                        this._mergedSettings[name] && (userOptions = $.extend(true, this._mergedSettings[name], userOptions));
            ...
            return {
                    ...
                    resetOptions: function(name) {
                        name && name !== "title" && (this._mergedSettings[name] = null)
                    },
                    update: function(options) {
    ...
    (function($, DX, undefined) {
        ...
        DX.viz.core.BaseWidget = DX.DOMComponent.inherit({
            ...
            _defaultOptions: function() {
                return $.extend(true, this.callBase(), {incidentOccured: function(options) {
        ...
        DX.viz.core.CoreFactory = function() {
            return {
                    ...
                    createTranslator1D: function(fromValue, toValue, fromAngle, toAngle) {
                        return new core.Translator1D(fromValue, toValue, fromAngle, toAngle)
                    },
