        ...
        DX.viz.charts.ChartTitle = DX.Class.inherit({
            ...
            getLayoutOptions: function() {
                ...
                return {
                        ...
                        cutLayoutSide: options.verticalAlignment
                    }
            },
            ...
            getBoundingRect: function() {
                return this.innerTitleGroup ? this.boundingRect : {
                        width: 0,
                        height: 0,
                        x: 0,
                        y: 0
            ...
            shift: function(x, y) {
                this.innerTitleGroup && this.innerTitleGroup.move(x, y);
    ...
    (function($, DX, undefined) {
        ...
        $.extend(DX.viz.charts.Axis.prototype, {
            ...
            _drawAxis: function() {
                ...
                if (!options.visible)
                    return;
                p11 = hasCategories ? translator.translateSpecialCase(CANVAS_POSITION_START) : translator.translate(options.min);
                p21 = hasCategories ? translator.translateSpecialCase(CANVAS_POSITION_END) : translator.translate(options.max);
                if (options.isHorizontal)
            ...
            _getTicksOptions: function() {
                var that = this,
                    ...
                    hasCategories = $.isArray(options.categories),
                    maxTicksCount = Math.round(screenDelta / options.label.minSpacing),
                    interval,
                    customTicks = hasCategories ? options.categories : that._tickValues;
                ...
                if (hasCategories && customTicks.length > maxTicksCount) {
                    interval = Math.round(customTicks.length / maxTicksCount);
                    customTicks = $.map(customTicks, function(item, i) {
                        return i % interval === 0 ? item : null
                    })
                }
                return {
                        min: min,
        ...
        charts.Chart = charts.BaseChart.inherit({
            ...
            _createPanes: function() {
                ...
                if (panes && utils.isArray(panes) && !panes.length || $.isEmptyObject(panes))
                    panes = that._defaultOptions().panes;
    ...
    (function($, DX, undefined) {
        ...
        DX.viz.charts.LayoutManager = DX.Class.inherit(function() {
            ...
            var placeElementAndCutCanvas = function(elements, canvas) {
                    $.each(elements, function(_, element) {
                        ...
                        switch (options.horizontalAlignment) {
                            case"left":
                                shiftX = canvas.left - box.x;
                                break;
                            case"center":
                                shiftX = (canvas.width - canvas.left - canvas.right - box.width) / 2 + canvas.left - box.x;
                                break;
                            case"right":
                                shiftX = canvas.width - canvas.right - box.x - box.width;
                                break
                        }
                        switch (options.verticalAlignment) {
                            case"top":
                                shiftY = canvas.top - box.y;
                                break;
                            case"bottom":
                                shiftY = canvas.height - canvas.bottom - box.height - box.y;
                                break
                        }
                        element.shift(round(shiftX), round(shiftY));
                        canvas[options.cutLayoutSide] += box[length];
            ...
            var placeDrawnElements = function(canvas) {
                    var that = this;
                    $.each(that.horizontalElements, function(_, element) {
                        var box = element.getBoundingRect();
                        var side = element.getLayoutOptions().cutLayoutSide;
                        canvas[side] -= box.width
                    });
                    placeElementAndCutCanvas(that.horizontalElements, canvas);
                    $.each(that.verticalElements, function(_, element) {
                        var box = element.getBoundingRect();
                        var side = element.getLayoutOptions().cutLayoutSide;
                        canvas[side] -= box.height
                    });
                    placeElementAndCutCanvas(that.verticalElements, canvas)
                };
            ...
            var updateDrawnElements = function(argumentAxis, valueAxis, canvas, dirtyCanvas, panes, rotated) {
                    ...
                    function updateElements(elements, length, otherLength) {
                        $.each(elements, function(_, element) {
                            var box = element.getBoundingRect(),
                                side = element.getLayoutOptions().cutLayoutSide,
                            ...
                            canvas[side] -= box[length] - element.getBoundingRect()[length];
                            needRemoveSpace[length] -= box[length] - element.getBoundingRect()[length]
                        })
                    }
                };
